classdef CannyEdgeDetection<handle
    properties
        guassSize
        sigma
        img
    ...
    methods
        function obj=CannyEdgeDetection(guassSize,sigma,img)
...
%             imshow(mat2gray(obj.ImgGaussianFilter),'InitialMagnification','fit')
        ...
        function GradientCalculation(obj)
            ...
            for x = 2:obj.length-1
                for y = 2:obj.width-1
                    Ix=sum(sum(Kx.*obj.ImgGaussianFilter(x-1:x+1,y-1:y+1)));
                    Iy=sum(sum(Ky.*obj.ImgGaussianFilter(x-1:x+1,y-1:y+1)));
